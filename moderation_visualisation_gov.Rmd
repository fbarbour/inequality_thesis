...
frm <- c(rep(" ~ log_carbon + govEffectiveness + log_carbon:govEffectiveness", 6), 
         rep(" ~ log_MF + govEffectiveness +log_MF:govEffectiveness", 6))
...
for (i in 1:2) {
  for (j in 1:6) {
    ...
    x.y <- jefim_data %>% select(foot_log[i], needs[j], govEffectiveness, foot_raw[i], needs_raw[j]) %>% mutate(footprint = foot_raw[i], need = needs_raw[j])
    
    colnames(x.y) <- c("log_footprint_col", "needs_col", "govEffectiveness", "raw_footprint_col", "raw_needs_col", "footprint", "need")
    ...
    x.y <- x.y %>% mutate(marginal.x = coeff_2 + coeff_4*govEffectiveness) %>% 
      mutate(marginal.inq = (coeff_3 + coeff_4*log_footprint_col)) %>% 
      mutate(sigma_marg.x = sqrt(var_2 + var_4*govEffectiveness^2 + 2*cov_2_4*govEffectiveness)) %>% 
...
```{r}
histogram %>% filter(significance == "sig" & d_NS >= -1 & d_NS <= 1) %>% select(d_NS) %>% as.matrix() %>% as.data.frame() %>% ggplot(aes(d_NS)) + geom_histogram(binwidth = 0.1, color="black", fill = "green", boundary = 0, boundary = 0) + geom_vline(xintercept=0, linetype="dashed") + geom_vline(xintercept=0, linetype="dashed") + xlim(-1,1) + ggtitle("Income inequality") + theme(legend.position="none") + ylab("Frequency") + xlab("NS improvement")

hist_carbon <- histogram %>% filter(significance == "sig" & d_NS >= -1 & d_NS <= 1 & footprint == "carbon_perCap_scaled") %>% select(d_NS) %>% as.matrix() %>% as.data.frame() %>% ggplot(aes(d_NS)) + geom_histogram(binwidth = 0.1, color="black", fill = "green", boundary = 0, boundary = 0) + geom_vline(xintercept=0, linetype="dashed") + geom_vline(xintercept=0, linetype="dashed") + xlim(-1,1) + ggtitle("Carbon footprint") + theme(legend.position="none") + ylab("Frequency") + xlab("NS improvement")

hist_MF <- histogram %>% filter(significance == "sig" & d_NS >= -1 & d_NS <= 1 & footprint == "MaterialFootprint") %>% select(d_NS) %>% as.matrix() %>% as.data.frame() %>% ggplot(aes(d_NS)) + geom_histogram(binwidth = 0.1, color="black", fill = "green", boundary = 0, boundary = 0) + geom_vline(xintercept=0, linetype="dashed") + geom_vline(xintercept=0, linetype="dashed") + xlim(-1,1) + ggtitle("Material footprint") + theme(legend.position="none") + ylab("Frequency") + xlab("NS improvement")
