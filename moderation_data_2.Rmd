...
```{r}
jefim_data <- Fanning2021_ecological %>% left_join(Fanning2021_social) %>% left_join(swiid_gini) %>% left_join(WID_wealth) %>% left_join(WB_furtherNeeds) %>% left_join(foodSecurity) %>% left_join(eora_carbon) %>% left_join(UN_pop) %>% left_join(pwt_PPP) %>% 
...
jefim_data <- jefim_data %>% mutate_at(c("life_sat", "nour_sat", "water_sat", "san_sat", "foodSec_sat", "energy_sat", "gini_disp", "gini_wealth", "log_MF", "log_carbon", "log_PPP"), ~(scale(.) %>% as.vector)) %>% 
  select(Country.Code, log_carbon, log_MF, life_sat, nour_sat, water_sat, san_sat, foodSec_sat, energy_sat, gini_disp, gini_wealth, log_PPP, carbon_perCap_scaled, MaterialFootprint, LifeExpectancy, Sanitation, AccessToEnergy, foodSecurity_scaled, nourishment_scaled, water_scaled)
```
