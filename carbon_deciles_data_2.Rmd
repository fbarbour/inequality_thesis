...
carbon_deciles <- Bruckner_deciles %>% merge(swiid_gini) %>% merge(dorninger) %>% merge(ECI) %>% merge(WID_wealth) %>% merge(urbanisation) %>% merge(popDensity) %>% merge(active_pop) %>% 
  mutate(log_CF = log(CF_decile), log_expend = log(expend_decile), log_popDensity = log(pop_density))

carbon_deciles_st <- carbon_deciles %>% mutate_at(c("log_CF", "log_expend", "gini_disp", "log_popDensity", "urbanisation", "tech_index", "ECI", "gini_wealth", "active_pop"), ~(scale(.) %>% as.vector))
```